...
print("\n" + "="*50)
print(" KỊCH BẢN 2: BẦU CUA - NHÀ CÁI VỐN LỚN")
print("="*50)

script_dir = os.path.dirname(os.path.abspath(__file__))
data_dir = os.path.join(script_dir, '..', 'data')
file_list = glob.glob(os.path.join(data_dir, 'data_lon_*.csv'))
...
print(f"Dang xu ly {len(file_list)} luong du lieu...")
for f in file_list:
    df = pd.read_csv(f)
    all_dfs.append(df)
...
for df in all_dfs:
    ...
    if l < max_rounds:
        sum_von_chungthuy[l:] += v_ct[-1]
        sum_von_raitham[l:] += v_rt[-1]
        sum_von_nhacai[l:] += v_nc[-1]
...
ax1.plot(df_short['Van'], df_short['Von_ChungThuy'], color='#d62728', label='Chung Thủy (Gấp Thếp 1 Cửa)', linewidth=1.5)
ax1.plot(df_short['Van'], df_short['Von_RaiTham'], color='#1f77b4', label='Rải Thảm (Chia 3 Cửa)', alpha=0.6)
ax1.plot(df_short['Van'], df_short['Von_NhaCai'], color='black', label='Nhà Cái', linestyle='--', linewidth=2)
ax1.set_title(f"NHANH NHẤT: NGƯỜI CHƠI CHÁY TÚI SỚM ({len(df_short):,} ván)", fontsize=16, fontweight='bold', color='red', pad=15)
...
ax2.set_title(f"LÂU NHẤT: NGƯỜI CHƠI MAY MẮN TRỤ LẠI ({len(df_long):,} ván)", fontsize=16, fontweight='bold', color='green', pad=15)
...
fig.suptitle('KỊCH BẢN 2: BẦU CUA - NHÀ CÁI VỐN LỚN', fontsize=22, fontweight='bold')
print("Hoan tat! Bieu do da duoc hien thi.")
plt.show()